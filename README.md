# DeepLearning.AI Short Courses
1. [Building Systems with the ChatGPT API](https://learn.deeplearning.ai/chatgpt-building-system)
1. [ChatGPT Prompt Engineering for Developers](https://learn.deeplearning.ai/chatgpt-prompt-eng)
1. [LangChain for LLM Application Development](https://learn.deeplearning.ai/langchain)
1. [LangChain Chat with Your Data](https://learn.deeplearning.ai/langchain-chat-with-your-data)
1. [How Diffusion Models Work](https://learn.deeplearning.ai/diffusion-models)

---

[Click here to see all of our courses and specializations](https://www.deeplearning.ai/courses/)